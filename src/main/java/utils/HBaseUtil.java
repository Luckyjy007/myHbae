...
/**
 * HBase 工具类
 * Created by jiangyun on 2017/12/29.
 */
...
//         System.out.println("---------- " + conn.hashCode());
    ...
    public static void createTable(String tableName, String[] columnFamilies, boolean preBuildRegion) throws Exception {
        if (preBuildRegion) {
            String[] s = new String[]{"1", "2", "3", "4", "5", "6", "7", "8", "9", "A", "B", "C", "D", "E", "F", "G"};
            int partition = 16;
...
//				conn.close();
    ...
    //get range rows
    public static ResultScanner getRowByRange(String tableName, String startRowKey, String endRowKey) throws IOException {
        Table table = getTable(tableName);
        Scan scan = new Scan();
    ...
    public static Scan preCount(String startRowKey, String endRowKey) throws IOException {
       // Table table = getTable(tableName);
        System.out.println("preCounting.............");
        Scan scan = new Scan();
        scan.setStartRow(Bytes.toBytes(startRowKey));
        scan.setStopRow(Bytes.toBytes(endRowKey));
        System.out.println("retrun scan.............");
        return scan;

    }

    //get range rows
    public static ResultScanner getRowFromStart(String tableName, String startRowKey) throws IOException {
        Table table = getTable(tableName);
        Scan scan = new Scan();
        scan.setStartRow(Bytes.toBytes(startRowKey));
        return table.getScanner(scan);
    ...
    //get range rows
    public static ResultScanner getRowUntilEnd(String tableName, String endRowKey) throws IOException {
        Table table = getTable(tableName);
        Scan scan = new Scan();
        scan.setStopRow(Bytes.toBytes(endRowKey));
        return table.getScanner(scan);
    ...
    //get single row
    public static Result getSingleRow(String tableName, String rowKey) throws IOException {
        Table table = getTable(tableName);
        Scan scan = new Scan();
...
//全表扫描
    public static ResultScanner getAll(String tablename) {
        Table table = getTable(tablename);
    ...
    public static Put objectToPut(Object object, String[] cf) throws IllegalAccessException, NoSuchFieldException {
        Field primaryKey = object.getClass().getDeclaredField("primaryKey");
        primaryKey.setAccessible(true);
        String pk = primaryKey.get(object).toString();
        Put put = new Put(Bytes.toBytes(System.currentTimeMillis() + pk));
        for (Field field : object.getClass().getDeclaredFields()) {
            field.setAccessible(true);
            put.addColumn(Bytes.toBytes(cf[0]), Bytes.toBytes(field.getName()), Bytes.toBytes(field.get(object).toString()));
        }
    ...
    public static List<String> resultScannerToList(ResultScanner results) {

        List<String> list = new ArrayList<String>(20000);
        for (Result result : results) {
            List<Cell> cells = result.listCells();
            String str = "rowKey=" + Bytes.toString(result.getRow()) + ",Family=CF,";
            for (Cell cell : cells) {

                str += Bytes.toString(CellUtil.cloneQualifier(cell)) + "=" + Bytes.toString(CellUtil.cloneValue(cell)) + ",";
            }
            list.add(str.trim().substring(0, str.length() - 1));
        }
        return list;
    }
    ...
    //统计行数
    public static Long resultScannerCount(ResultScanner results) {
    ...
    public static long rowCount2(String tableName ,Scan scan) throws IllegalArgumentException, Throwable {
        ...
        boolean flag = htd.hasCoprocessor(coprocessorClassName);
        if (!flag) {
            admin.disableTable(tableName);
